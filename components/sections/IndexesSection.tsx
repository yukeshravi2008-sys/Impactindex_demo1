        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {indexes.map((index) => {
            const avgImpact = Math.round(
              index.ngos.reduce((s, n) => s + n.impactScore, 0) /
                index.ngos.length,
            );
            ...
            return (
              <Card
                key={index.slug}
                className="group flex flex-col overflow-hidden rounded-2xl border-stone-100 shadow-sm transition-shadow hover:shadow-md"
              >
                ...
                <div className="border-t border-stone-100 p-4">
                  <Button
                    render={
                      <Link href={`/donate?index=${index.slug}`} />
                    }
                    variant="ghost"
